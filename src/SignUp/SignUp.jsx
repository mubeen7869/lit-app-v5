...
//   };
  ...
  return (
    ...
    <p className="error-message">first name should not contain spaces</p>
)}
...
{lastname.length > 0 && lastname.includes(' ') && (
    <p className="error-message">last name should not contain spaces</p>
)}
...
{password && !/^(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[!@#$%^&*()-_=+[\]{}|;:'",.<>?]).{8,}$/.test(password) && (
  <p className="error-message">Please enter a valid password: at least 8 characters, one uppercase letter, one lowercase letter, one number, and one special character</p>
)}
